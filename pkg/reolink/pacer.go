...
// pacer code based on reolinkproxy's pacer
...
func (p *mediaPacer) warnOverflowOnce() {
	...
	if p.logf != nil {
		p.logf("media pacer queue overflow (cap=%d); dropping frame", cap(p.ch))
	}
}
...
// packet via the frame's write closure and advances the cursor by duration.
